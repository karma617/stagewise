...
## 2026-07-23 - Task: build-fast only packages PickStar Studio
### What was done
- Updated `build-fast.bat` to close running `pickstar-studio.exe`, `stagewise.exe`, and `stagewise-dev.exe` processes before packaging.
- Added stale output cleanup before packaging so old PickStar, legacy Stagewise, and old `release/make` artifacts do not get reused or distributed.
- Restricted Camoufox resource copying and zip creation to `pickstar-studio-*` packaged app directories only.
- Updated the Windows fast-build documentation to describe the new process and PickStar Studio artifact paths.

### Testing
- `cmd /c "echo y|build-fast.bat"` initially exposed a no-process close-command exit-code issue; after fixing it, the command exited 0.
- Final `cmd /c "echo y|build-fast.bat"` exited 0 and generated only `apps/browser/out/dev/pickstar-studio-win32-x64` plus `apps/browser/out/dev/pickstar-studio-win32-x64.zip`.
- Checked `apps/browser/out` recursively for `stagewise*` / `pickstar-studio*` win32 directories and zip packages; only the PickStar Studio dev directory and zip remained.
- `git diff --check -- build-fast.bat docs/windows-build-fast.md progress.md` exited 0.

### Notes
- `build-fast.bat`: added pre-build app process shutdown, stale output cleanup, PickStar-only resource copy, and PickStar-only zip filtering. Rollback: `git checkout -- build-fast.bat`.
- `docs/windows-build-fast.md`: updated fast-build steps and artifact paths for PickStar Studio. Rollback: `git checkout -- docs/windows-build-fast.md`.
- `apps/browser/out/dev/pickstar-studio-win32-x64`: regenerated by the validation run. Rollback: delete the directory and rerun the desired packaging command.
- `apps/browser/out/dev/pickstar-studio-win32-x64.zip`: regenerated by the validation run. Rollback: delete the zip and rerun the desired packaging command.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-23 - Task: make goal objective editor multiline
### What was done
- Changed the goal status card objective editor from a single-line input to a multiline textarea.
- Kept the existing save/cancel buttons and changed keyboard behavior so Enter inserts a newline, Ctrl+Enter or Cmd+Enter saves, and Escape cancels.
- Removed the two-line clamp only while editing so the textarea can expand vertically.

### Testing
- `pnpm exec biome check --formatter-enabled=false apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx` exited 0.
- `pnpm -F stagewise exec tsc -p tsconfig.ui.json --noEmit` exited 0.
- `git diff --check -- apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx progress.md` exited 0; output only CRLF/LF normalization warning for `progress.md`.

### Notes
- `apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx`: replaced the goal objective edit input with a resizable multiline textarea and adjusted Enter/Ctrl+Enter behavior. Rollback: `git checkout -- apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx`.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-23 - Task: stabilize goal objective editor focus
### What was done
- Marked the goal status card with `data-status-card` so the chat input focus manager treats the goal editor like other footer status-card inputs instead of stealing focus back.
- Added an explicit textarea ref and post-render focus retry when entering goal edit mode, so active goal rerenders do not leave the editor unfocused.
- Kept the multiline textarea behavior from the previous change: Enter inserts a newline, Ctrl+Enter or Cmd+Enter saves, and Escape cancels.
...
### Notes
- `apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx`: added the missing status-card marker and explicit focus retry for the goal objective textarea. Rollback: `git checkout -- apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx`.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-23 - Task: add tray close-to-tray behavior
### What was done
- Added a persistent PickStar Studio tray icon with a localized context menu for show, hide, and real Exit/退出.
- Changed the main-window close flow so clicking the window close button saves state and hides the window to the tray instead of quitting the app.
- Kept real shutdown paths intact by allowing app quit/update quit/tray Exit to pass through the existing graceful shutdown flow.
- Documented the new tray behavior in the PickStar Studio branding notes.

### Testing
- `pnpm exec biome check --formatter-enabled=false apps/browser/src/backend/index.ts apps/browser/src/backend/main.ts apps/browser/src/backend/services/window-layout/index.ts apps/browser/src/backend/services/app-tray.ts docs/pickstar-studio-branding.md` exited 0.
- `pnpm -F stagewise exec tsc -p tsconfig.backend.json --noEmit` exited 0.
- `git diff --check -- apps/browser/src/backend/index.ts apps/browser/src/backend/main.ts apps/browser/src/backend/services/window-layout/index.ts apps/browser/src/backend/services/app-tray.ts docs/pickstar-studio-branding.md` exited 0; output only CRLF/LF normalization warnings.

### Notes
- `apps/browser/src/backend/services/app-tray.ts`: added the Electron tray service, tray tooltip, click/double-click restore, and right-click context menu with Exit/退出. Rollback: delete this file and remove its wiring from `apps/browser/src/backend/main.ts`.
- `apps/browser/src/backend/services/window-layout/index.ts`: added show/hide/quit helpers and close interception so normal window close hides to tray while real app quit continues. Rollback: `git checkout -- apps/browser/src/backend/services/window-layout/index.ts`.
- `apps/browser/src/backend/main.ts`: wired `AppTrayService` into startup and graceful shutdown teardown. Rollback: `git checkout -- apps/browser/src/backend/main.ts`.
- `apps/browser/src/backend/index.ts`: stopped auto-quitting on `window-all-closed` so hidden-to-tray sessions remain alive. Rollback: `git checkout -- apps/browser/src/backend/index.ts`.
- `docs/pickstar-studio-branding.md`: documented the close-to-tray and tray Exit behavior. Rollback: `git checkout -- docs/pickstar-studio-branding.md`.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-24 - Task: fix custom provider context window enforcement
### What was done
- Made request preflight compression more conservative by lowering the send-before-compress threshold from 85% to 75% of the active model context window, leaving room for provider-side tokenization and tool-schema overhead.
- Extended context-window recovery detection to catch upstream errors that say `maximum prompt length` / `request contains ... tokens`, so those errors trigger compression-and-retry instead of immediately stopping the chat.
- Fixed the chat footer token badge for built-in models routed through custom providers, so it uses the custom endpoint `contextWindowSize` instead of the built-in model's catalog window.
- Added regression coverage for Anthropic built-in model routing through a custom OpenAI-compatible endpoint with a 480000-token context override.
- Updated the context-compression documentation to match the new 75% preflight threshold and the additional upstream error pattern.

### Testing
- `pnpm exec biome check --formatter-enabled=false packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/shared/context-compression-thresholds.ts packages/agent-core/src/agents/shared/context-compression-thresholds.test.ts apps/browser/src/backend/agents/model-provider.test.ts apps/browser/src/ui/screens/main/agent-chat/chat/_components/message-assistant.tsx apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx docs/agent-context-compression.md` exited 0.
- `pnpm -F @stagewise/agent-core test -- context-compression-thresholds` exited 0.
- `pnpm -F stagewise test -- model-provider` exited 0.
- `pnpm -F stagewise exec tsc -p tsconfig.backend.json --noEmit` exited 0.
- `pnpm -F stagewise exec tsc -p tsconfig.ui.json --noEmit` exited 0.
- `pnpm -F @stagewise/agent-core exec tsc --noEmit` exited 0.
- `git diff --check -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/shared/context-compression-thresholds.ts packages/agent-core/src/agents/shared/context-compression-thresholds.test.ts apps/browser/src/backend/agents/model-provider.test.ts apps/browser/src/ui/screens/main/agent-chat/chat/_components/message-assistant.tsx apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx docs/agent-context-compression.md` exited 0; output only CRLF/LF normalization warnings.

### Notes
- `packages/agent-core/src/agents/shared/context-compression-thresholds.ts`: changed the shared preflight fraction from 0.85 to 0.75. Rollback: restore this file and its test together.
- `packages/agent-core/src/agents/shared/context-compression-thresholds.test.ts`: updated the preflight threshold expectation to 750000 for a 1M context window. Rollback: restore this file with the threshold file.
- `packages/agent-core/src/agents/base-agent.ts`: added `maximum prompt length` / `request contains` matching to the context-window recovery detector. Rollback: remove those two match branches.
- `apps/browser/src/ui/screens/main/agent-chat/chat/_components/message-assistant.tsx`: changed the visible preflight limit summary from 85% to 75%. Rollback: change the multiplier back if reverting the threshold.
- `apps/browser/src/ui/screens/main/agent-chat/chat/_components/panel-footer.tsx`: changed only the footer token max calculation to honor custom endpoint context overrides for routed built-in models; this file already had unrelated prior goal-editor edits in the working tree. Rollback: remove the added `customEndpoints` / `providerConfigs` selectors and the custom-endpoint branch in `maxTokens`.
- `apps/browser/src/backend/agents/model-provider.test.ts`: added Anthropic custom-endpoint context override coverage. Rollback: delete the new `uses custom endpoint context window for routed Anthropic built-in models` test.
- `docs/agent-context-compression.md`: documented the new upstream error pattern and 75% preflight threshold. Rollback: revert the changed bullets in this document.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-24 - Task: force compression when continuing oversized context
### What was done
- Added a synchronous preflight guard that checks token usage since the latest compression checkpoint before every model request. If an already-over-limit conversation continues or retries, the agent now compresses history first and only then rebuilds the request context.
- Extended context-window error detection to inspect the provider response body, so generic `Upstream error: 400` wrappers with a hidden `maximum prompt length` message are treated as context overflow and trigger compression recovery.
- Added regression tests for generic upstream 400 errors whose response body contains the actual prompt-length overflow message.
- Updated the context-compression documentation to describe the new checkpoint-based preflight guard and response-body error detection.

### Testing
- `pnpm exec biome check --formatter-enabled=false packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` exited 0.
- `pnpm -F @stagewise/agent-core test -- chat.test.ts` exited 0; 8 tests passed.
- `pnpm -F @stagewise/agent-core exec tsc --noEmit` exited 0.
- `pnpm -F stagewise exec tsc -p tsconfig.backend.json --noEmit` exited 0.
- `git diff --check -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` exited 0; output only CRLF/LF normalization warnings.

### Notes
- `packages/agent-core/src/agents/base-agent.ts`: added checkpoint-based preflight compression and response-body context-overflow detection. Rollback: remove the `context-preflight-usage-compression` block and restore `isContextWindowExceededError` to message-only matching.
- `packages/agent-core/src/agents/chat/chat.test.ts`: added regression tests for prompt-length overflow hidden behind generic upstream 400 errors. Rollback: delete the `ChatAgent context-window error detection` describe block.
- `docs/agent-context-compression.md`: documented the forced preflight compression and response-body error matching behavior. Rollback: revert this document's runtime behavior bullets.
- `progress.md`: appended this task record. Rollback: delete this appended record.

## 2026-07-24 - Task: 修复目标模式超上下文仍直接请求模型的问题
### What was done
- 在模型请求前增加工具构建后的最终请求体预检，最终预检会把模型消息、工具 JSON Schema 和请求包装开销一起估算，超过预检阈值时先同步压缩并重建上下文，再进入等待模型响应阶段。
- 修复旧会话中已有压缩摘要但没有 compressionState baseline 时把当前 usedTokens 当作基线的问题，避免已超限目标会话恢复后跳过预检压缩。
- 增加回归用例覆盖旧压缩摘要 baseline、上下文错误识别以及工具 schema 纳入最终请求估算，并同步更新上下文压缩说明文档。
### Testing
- `pnpm exec biome check --formatter-enabled=false packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` 通过。
- `pnpm -F @stagewise/agent-core test -- chat.test.ts` 通过，11 个测试全部通过。
- `pnpm -F @stagewise/agent-core exec tsc --noEmit` 通过。
- `pnpm -F stagewise exec tsc -p tsconfig.backend.json --noEmit` 通过。
- `git diff --check -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` 通过，仅提示现有 CRLF/LF 行尾警告。
### Notes
- `packages/agent-core/src/agents/base-agent.ts`：补最终请求体预检、工具 schema token 估算、旧压缩摘要 baseline 修正。
- `packages/agent-core/src/agents/chat/chat.test.ts`：补最终请求估算和旧 baseline 的回归测试。
- `docs/agent-context-compression.md`：补充最终请求体预检和旧压缩记录处理说明。
- 回滚方式：使用 `git checkout -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` 回退本轮改动，或按上述文件从当前 diff 中反向应用本轮补丁。
- `progress.md`：追加本轮任务记录。回滚方式：删除本轮 `2026-07-24 - Task: 修复目标模式超上下文仍直接请求模型的问题` 记录。

## 2026-07-24 - Task: 修复目标模式仍不显示上下文压缩的问题
### What was done
- 将上下文窗口读取改为使用当前 agent 实例和 agent-step 路由，并在预检中优先使用本轮模型解析出来的 `contextWindowSize`，避免后端按默认大窗口计算而跳过压缩。
- 在最终请求体预检中补充压缩失败或压缩后仍超限的兜底：自动切换为 lean context，跳过文件上下文、路径引用展开、环境捕获和技能注入后再请求，避免当前目标中的大 `.textclip`/附件全文继续撑爆上下文。
- 增加回归测试，确认 context window 会带当前 agent 实例读取，同时保留工具 schema 估算和旧压缩 baseline 测试。
- 更新上下文压缩文档，说明自定义服务商窗口对齐与 lean context 兜底行为。
### Testing
- `pnpm exec biome check --formatter-enabled=false packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md` 通过。
- `pnpm -F @stagewise/agent-core test -- chat.test.ts` 通过，12 个测试全部通过。
- `pnpm -F @stagewise/agent-core exec tsc --noEmit` 通过。
- `pnpm -F stagewise exec tsc -p tsconfig.backend.json --noEmit` 通过。
- `git diff --check -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md progress.md` 通过，仅提示现有 CRLF/LF 行尾警告。
### Notes
- `packages/agent-core/src/agents/base-agent.ts`：修正 contextWindowSize 读取路径，最终预检压缩后仍超限时改用 lean context 并保持 `compressing-context` 阶段可见。
- `packages/agent-core/src/agents/chat/chat.test.ts`：补充当前 agent 实例读取 context window 的回归测试。
- `docs/agent-context-compression.md`：补充自定义服务商窗口对齐和 lean context 兜底说明。
- `progress.md`：追加本轮任务记录。
- 回滚方式：使用 `git checkout -- packages/agent-core/src/agents/base-agent.ts packages/agent-core/src/agents/chat/chat.test.ts docs/agent-context-compression.md progress.md` 回退本轮及相关未提交改动，或按当前 diff 反向移除本轮新增逻辑和记录。

## 2026-07-24 - Task: 修复 build-fast 未打入最新 agent-core 导致压缩日志缺失的问题
### What was done
- 分析用户提供的 `agent-runtime-2026-07-23.jsonl`，确认运行包没有 `final-request-preflight-*` 事件，且 `stream-request-start` 也缺少当前源码已有的 token/window 字段，说明实际运行的 asar 仍是旧构建产物。
- 修改 `build-fast.bat`，在 Electron 打包前强制执行 `pnpm --filter @stagewise/agent-core build`，并校验 `packages/agent-core/dist/agents/index.js` 必须包含 `final-request-preflight`，避免继续把旧 agent-core dist 打包进去。
- 将 Vite 缓存清理从耗时的 `pnpm clear-vite-cache` 改为直接删除固定缓存目录，并加入 browser 根目录边界校验，避免旧 workspace 包缓存复用。
- 更新 `docs/windows-build-fast.md`，说明 build-fast 会自动重建 agent-core、校验预检代码并清理 Vite 缓存。
### Testing
- `pnpm --filter @stagewise/agent-core build` 通过。
- `Select-String packages/agent-core/dist/agents/index.js -Pattern final-request-preflight` 通过，确认 dist 已包含最新预检代码。
- 解析 `C:\Users\karma617\Desktop\agent-runtime-2026-07-23.jsonl`：`final-request-preflight*` 事件为 0，且 `stream-request-start` 没有 `contextWindowTokens` 字段，确认问题来自旧包未打入最新代码。
- `git diff --check -- build-fast.bat docs/windows-build-fast.md progress.md` 通过，仅提示现有 CRLF/LF 行尾警告。
### Notes
- `build-fast.bat`：打包前重建并校验 agent-core dist，直接清理 Vite cache，防止 asar 使用旧核心包。
- `docs/windows-build-fast.md`：同步 build-fast 新流程说明。
- `progress.md`：追加本轮任务记录。
- 回滚方式：使用 `git checkout -- build-fast.bat docs/windows-build-fast.md progress.md` 回退本轮脚本、文档和日志记录改动；如只回退脚本逻辑，可从 `build-fast.bat` 删除 agent-core build/校验和 Vite cache 直接删除步骤。
