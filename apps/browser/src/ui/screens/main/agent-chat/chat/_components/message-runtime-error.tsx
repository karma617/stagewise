...
/** Detect transient network/connection errors that are safe to auto-retry */
function isNetworkConnectionError(error: GenericRuntimeError): boolean {
  const msg = error.message.toLowerCase();
  return (
...
function GenericError({
  ...
  // Auto-retry transient network errors up to GENERIC_AUTO_RETRY_MAX times
  useEffect(() => {
  ...
  return (
    <div className="mt-6 flex w-full flex-col gap-1.5 rounded-lg border border-derived-strong p-2 text-sm">
      ...
      {autoRetryMsg && (
        <div className="text-muted-foreground text-xs">{autoRetryMsg}</div>
      )}
