...
// FAST_BUILD=1: 本地快速打包模式
// - 跳过原生模块强制 rebuild
// - 由 vite.*.config.ts 读取以跳过 sourcemap 生成
// CI 不设置此变量，行为完全不变。
...
const dmgVolumeName = 'Install PickStar Studio';
...
const hardlinkRecursive = (src: string, dest: string): void => {
  ...
  if (stat.isSymbolicLink()) {
    // 软链接按原内容 copy，避免链接到 monorepo 外部丢失目标
    fs.cpSync(src, dest);
    return;
  }
  try {
...
const uploadSourceMapsAndCleanup = (
  ...
  try {
    ...
    execSync(
      `posthog-cli sourcemap upload --directory "${viteDir}" --release-name pickstar-studio --release-version "${version}"`,
      { stdio: 'inherit' },
    );
...
const config: ForgeConfig = {
  ...
  packagerConfig: {
    ...
    appCopyright: buildConstants.__APP_COPYRIGHT__,
    win32metadata: {
      CompanyName: buildConstants.__APP_AUTHOR__,
  ...
  makers: [
    new MakerSquirrel((arch) => ({
      ...
      copyright: buildConstants.__APP_COPYRIGHT__,
